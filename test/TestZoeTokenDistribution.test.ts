...
describe("Token contract", async () => {
    ...
    it("3. Add Liquidity", async () => {
        ...
        const amountToken = parseEther('60000000');
        const amountBNB = parseEther('1');
    ...
    it("5. Transfer From Owner To Bob ", async () => {
        await tokenDeployed.transfer(bob.address, parseEther("20001"))
        expect(await tokenDeployed.balanceOf(bob?.address)).to.be.eq(parseEther("20001"));
        console.log()
    });

    it("6. Transfer From Bob To Alice ", async () => {
        await tokenDeployed.connect(bob).transfer(alice?.address, parseEther("9999"))
        expect(await tokenDeployed.balanceOf(alice?.address)).to.be.eq(parseEther("9999"));
        console.log()
    });
    ...
    it("7. Buy Bob", async () => {
        ...
        console.log()
        //--- BUY
        console.log(`${colors.cyan('Bob token Balance Before Swap')}: ${colors.yellow(formatEther(await tokenDeployed.balanceOf(bob.address)))}`)
        ...
        console.log(`${colors.cyan('Contract token Balance After Swap')}: ${colors.yellow(formatEther(await tokenDeployed.balanceOf(tokenDeployed.address)))}`)
